...
@patch("hotkey_mapping.xdotool_wrapper.get_display_size")
def test_normalize_target_primary_screen_doesnt_change_anything(
    mock_get_display_size: MagicMock,
):
    mock_get_display_size.return_value = Vector2(2560, 1440)
    start = Vector2(1000, 1000)
    target = Vector2(2000, 1200)
    expected = target
...
def test_normalize_target_second_screen_changes_just_x(
    ...
    mock_get_display_size.return_value = Vector2(2400, 1440)
    start = Vector2(2500, 1000)
    target = Vector2(2000, 1200)
    expected = Vector2(4400, 1200)
    actual = normalize_target(start, target)
    assert actual == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ParamGetDuration(
            ...
            display_size=Vector2(1000, 1000),
            expected=0.5,
        ),
        ...
        # Super not rounded
        ParamGetDuration(
            start=Vector2(321, 654),
            target=Vector2(123, 456),
            screens_per_second=0.33,
            display_size=Vector2(777, 999),
...
@pytest.mark.parametrize(
    ...
    [
        ParamVector2Magnitude(x=3, y=4, expected=5),
        ParamVector2Magnitude(x=33, y=44, expected=55),
        ParamVector2Magnitude(x=3, y=3, expected=4.242640),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ParamVector2Lerp(
            ...
            percent=0.9,
            expected=Vector2(45, 90),
        ),
    ],
)
def test_Vector2_lerp(
    start: Vector2, target: Vector2, percent: float, expected: Vector2
):
    actual = start.lerp(target, percent)
    assert_within_margin_of_error(actual, expected)


@patch("hotkey_mapping.xdotool_wrapper.get_mouse_location")
@patch("hotkey_mapping.xdotool_wrapper.normalize_target")
@patch("hotkey_mapping.xdotool_wrapper.get_duration")
@patch("hotkey_mapping.xdotool_wrapper.run_command")
@patch("hotkey_mapping.xdotool_wrapper.time.sleep")
def test_move_mouse_has_right_number_of_calls(
    ...
    mock_normalize_target.assert_called_with(
        mock_get_mouse_location.return_value, mock_target
    )
    mock_get_duration.assert_called_with(
        mock_start, mock_normalize_target.return_value, screens_per_second
    )
